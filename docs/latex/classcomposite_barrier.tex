...
703                                                                                                            
                                                          \{
704     \textcolor{keywordflow}{try} \{
705         \textcolor{keyword}{auto} barr = std::make\_shared < cylinderZ > (x, y, radius, width, sigma, eps, M);
706         sysBarriers\_.push\_back(barr);
707     \} \textcolor{keywordflow}{catch} (std::exception &ex) \{
708         \textcolor{keyword}{const} std::string msg = ex.what();
709         \textcolor{keywordflow}{throw} \hyperlink{classcustom_exception}{customException} (\textcolor{stringliteral}{"Cannot add cylinderZ to composite barrier : "}+msg);
710     \}
711 \}
\end{DoxyCode}
...
664         \textcolor{keyword}{auto} barr = std::make\_shared < hardWallZ > (lb, ub, sigma, M);
665         sysBarriers\_.push\_back(barr);
666     \} \textcolor{keywordflow}{catch} (std::exception &ex) \{
667         \textcolor{keyword}{const} std::string msg = ex.what();
668         \textcolor{keywordflow}{throw} \hyperlink{classcustom_exception}{customException} (\textcolor{stringliteral}{"Cannot add hardWallZ to composite barrier : "}+msg);
669     \}
670 \}
\end{DoxyCode}
...
730         \textcolor{keyword}{auto} barr = std::make\_shared < rightTriangleXZ > (width, theta, lamW, eps, sigma, sep, offset, box,
       zbase, top, M);
731         sysBarriers\_.push\_back(barr);
732     \} \textcolor{keywordflow}{catch} (std::exception &ex) \{
733         \textcolor{keyword}{const} std::string msg = ex.what();
734         \textcolor{keywordflow}{throw} \hyperlink{classcustom_exception}{customException} (\textcolor{stringliteral}{"Cannot add rightTriangleXZ to composite barrier : "}+msg);
735     \}
736 \}
\end{DoxyCode}
...
684         \textcolor{keyword}{auto} barr = std::make\_shared < squareWellWallZ > (lb, ub, sigma, range, eps, M);
685         sysBarriers\_.push\_back(barr);
686     \} \textcolor{keywordflow}{catch} (std::exception &ex) \{
687         \textcolor{keyword}{const} std::string msg = ex.what();
688         \textcolor{keywordflow}{throw} \hyperlink{classcustom_exception}{customException} (\textcolor{stringliteral}{"Cannot add squareWellWallZ to composite barrier : "}+msg);
689     \}
690 \}
\end{DoxyCode}
...
\begin{DoxyCode}
766                                                                                 \{
767     \textcolor{keywordtype}{double} U = 0.0;
768     \textcolor{keywordflow}{for} (std::vector < std::shared\_ptr < barrier > >::iterator it = sysBarriers\_.begin(); it != 
      sysBarriers\_.end(); ++it) \{
769         \textcolor{keywordtype}{double} dU = (*it)->energy (a1, box);
770         \textcolor{keywordflow}{if} (dU < \hyperlink{potentials_8h_ab94ab1d09e2291d03fe92a0e24a9d33b}{NUM\_INFINITY}) \{
771             U += dU;
772         \} \textcolor{keywordflow}{else} \{
773             \textcolor{keywordflow}{return} \hyperlink{potentials_8h_ab94ab1d09e2291d03fe92a0e24a9d33b}{NUM\_INFINITY};
774         \}
775     \}
776     \textcolor{keywordflow}{return} U;
777 \}
\end{DoxyCode}
