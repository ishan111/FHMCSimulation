...
tmmc::tmmc (const int Nmax, const int Nmin,  const int Mtot, const long long int tmmcSweepSize, const std::vector <double> box) {
	if (Nmin > Nmax) {
		throw customException ("Nmin ("+std::to_string(Nmin)+") > Nmax ("+std::to_string(Nmax)+" in TMMC bias");
	}
	if (Nmin < 0) {
...
void tmmc::updateC (const int Nstart, const int Nend, const int Mstart, const int Mend, const double pa) {
	...
	try {
		...
		throw customException ("Cannot update collection matrix: " + std::to_string(*ce.what()));
	}
	try {
...
/*!
 * Read the visited states matrix from a file.  This assumes the user has already guaranteed that the bounds are consistent,
...
/*!
 * Read the visited states histogram from a file.  This assumes the user has already guaranteed that the bounds are consistent,
 * e.g. Nmin and Nmax, as it will not check this automatically.  Also assumes file was generated by this code.  "Hand made"
 * ones might have formatting issues since parsing is done based on tokens.
 *
