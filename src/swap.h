...
public:
	...
	swapParticles (const int typeIndex1, const int typeIndex2, const std::string tag) { typeIndex_ = typeIndex1; typeIndex2_ = typeIndex2; name_ = tag + sstr(typeIndex1) + "_" + sstr(typeIndex2); changeN_ = false; } //!< Instantiate a new move, also give a name which is the combination of a user-defined tag + the particle indices it operates on
