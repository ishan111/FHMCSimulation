	...
	int  N_in_k = 0, N_in_j = 0, N_out_k = 0, N_out_j = 0;
    	...
    	if (rng(&RNG_SEED) < pBias_) {
			...
			std::vector < atom* > neighborAtoms;
					...
					const double d2 = pbc_dist2(sys.atoms[i][j].pos, sys.atoms[typeIndex2_][pkK].pos, sys.box());
			...
			while (inJ && iter < iterMax) {
        	        	...
        	        	if (&sys.atoms[ranSpec][ranIndex] != &sys.atoms[typeIndex_][pkJ] && &sys.atoms[ranSpec][ranIndex] != &sys.atoms[typeIndex2_][pkK]) {
                			const double d2 = pbc_dist2(sys.atoms[ranSpec][ranIndex].pos, sys.atoms[typeIndex_][pkJ].pos, sys.box());
		...
		N_in_j = 0;
        	for (unsigned int j = 0; j < totJatoms; ++j) {
        		const double d2 = pbc_dist2(sys.atoms[chosenAtomType][j].pos, sys.atoms[typeIndex_][pkJ].pos, sys.box());
			if (d2 < rc1max_*rc1max_ && d2 >= rc1min_*rc1min_) {
		...
		for (unsigned int i = 0; i < sys.nSpecies(); ++i) {
            		...
            		for (unsigned int j = 0; j < totJatoms; ++j) {
                		const double d2 = pbc_dist2(sys.atoms[i][j].pos, sys.atoms[typeIndex_][pkJ].pos, sys.box());
				if (d2 < rc1max_*rc1max_ && d2 >= rc1min_*rc1min_) {
		...
		if (rng (&RNG_SEED) < 0.5) {
	            	...
	            	while (inJ) {
	                	...
	                	// check that this position is "out" of J
        	       		const double d2 = pbc_dist2(tmpNewAtom.pos, sys.atoms[typeIndex_][pkJ].pos, sys.box());
				if (d2 >= rc1max_*rc1max_ || d2 < rc1min_*rc1min_) {
                		    	inJ = false;
        		        }
	            	}
			...
			if (sys.getCurrentM() > 0 && sys.getFractionalAtomType() == chosenAtomType) {
                	...
                	for (unsigned int j = 0; j < totKatoms; ++j) {
                        	const double d2 = pbc_dist2(sys.atoms[chosenAtomType][j].pos, sys.atoms[typeIndex2_][pkK].pos, sys.box());
				if (d2 < rc2max_*rc2max_ && d2 >= rc2min_*rc2min_) {
